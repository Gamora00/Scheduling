...
exports.UnarchiveTeacher = async (req, res) => {
    ...
    try {
        ...
        await connection.query(`
            INSERT INTO teacher (teacher_id, first_name, last_name, subject, teacher_username, teacher_password)
            SELECT teacher_id, first_name, last_name, subject, teacher_username, password
            FROM teacher_archive
            WHERE teacher_id = ?;
        `, [teacher_id]);

        // Delete teacher from original table
        await connection.query(`
            DELETE FROM teacher_archive
            WHERE teacher_id = ?;
        `, [teacher_id]);
